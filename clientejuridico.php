...
$(document).ready(function() {
  ...
  <header class="ls-info-header">
    <!--icone no titulo -->
    <h2 class="ls-title-3 ls-ico-user">Clientes Jurídicos</h2>
    <a href="cadastro_clientejuridico.php" class="ls-btn ">Adicionar Cliente</a>
  </header>
...
<table  class="ls-table ls-table-striped display" id="tabela" style="width:100%">
  <thead>
    <tr>
      <th class="ls-data-descending">CNPJ</th>
  ...
  <tbody>
    ...
    <tr>
     <?php
        ...
        do {
      ?>
      <td class="hidden-xs"><?=$linha['cnpj']?></td>
      ...
      <td>
        ...
        <form action="visualizar_cj.php" method="POST" enctype="multiparty/form-data">
        <input type="hidden" name="id" id="id" value=<?=$linha['id']?>> 
        <button type="submit" class="ls-btn ls-ico-eye espacobtn" title="Ver"></button>
        </form>
        <form action="editar_clientejuridico.php" method="POST" enctype="multiparty/form-data">
        <input type="hidden" name="id" id="id" value=<?=$linha['id']?>>
        <button  type="submit" class="ls-btn ls-ico-pencil espacobtn" title="Visualizar informações"></button>
        </form>
        <form action="include/Dcj.php" method="POST" enctype="multiparty/form-data">
        <input type="hidden"name="id" id="id" value=<?=$linha['id']?>>

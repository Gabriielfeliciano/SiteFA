...
<html class="ls-theme-indigo">
  ...
  <header class="ls-info-header">
    <h2 class="ls-title-3 ls-ico-user">Perfil - Administrador</h2>
  </header>
...
$idper = $_SESSION['idFunc'];

// cria a instrução SQL que vai selecionar os dados
$query = sprintf("SELECT  * FROM funcionarios WHERE idFuncionario = $idper");
...
$total = mysqli_num_rows($dados);
  ...
  <div class="divImg">
     <img src="img/<?=$linha['fotoPerfil']?>" >
     <form action="include/UimgAdm.php" method="post" enctype="multipart/form-data">
     <input type="file" name="arquivo" class="ls-btn">
     <input type="hidden" name="idFuncionario" value="1">
     <br><br>
     <input type="submit" name="" value="Mudar Foto" class="ls-btn-primary">
    </form>
  </div>
  <div class="form">
    <form action="include/UperFunc.php" method="post" class="ls-form-horizontal " data-ls-module="form">
      <fieldset id="fields-disabled" class="row ls-form-disable ls-form-text">
        ...
        <label class="ls-label col-md-3">
          ...
          <input type="text" id="cpf" name="cpf" value = <?=$linha['cpf']?> >
        </label>

        <label class="ls-label col-md-3">
          <b class="ls-label-text">Data de Nascimento</b>
          <input type="text" name="dataNascimento" id="dataNascimento" class="datepicker" value = <?=date("d/m/Y",strtotime(str_replace('-','/',$linha["dataNascimento"])));?>>
        </label>
        ...
        <input type="hidden" name="idFuncionario" value=<?=$linha['idFuncionario']?>>

...
$(document).ready(function() {
  ...
  <header class="ls-info-header">
    <!--icone no titulo -->
    <h2 class="ls-title-3 ls-ico-panel-emkt">Orçamentos</h2>
  </header>
...
<table  class="ls-table ls-table-striped display" id="tabela" style="width:100%">
  <thead>
    <tr>
      <th class="ls-data-descending">Email</th>
      <th class="hidden-xs ls-data-descending">Serviço</th>
      <th class="hidden-xs ls-data-descending">Necessidades</th>
  ...
  <tbody>
    ...
    <tr>
     <?php
      ...
      <td class="hidden-xs"><?=$linha['servico']?></td>
      <td class="hidden-xs"><?=$linha['necessidade']?></td>
      <td>
        ...
        <form action="include/Dorc.php" method="POST" enctype="multiparty/form-data">
        <input type="hidden"name="idOrcamento" id="idOrcamento" value=<?=$linha['idOrcamento']?>>

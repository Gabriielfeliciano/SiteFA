...
<html class="ls-theme-indigo">
  <head>
    <title>Feliciano Associados - Administrador</title>
...
$(document).ready(function() {
    $('#tabela').DataTable( {
      ...
      buttons: [
        ...
        { extend: 'pdf', className: 'ls-btn-pdf', orientation: 'landscape',  pageSize: 'A4', filename:'Feliciano Associados - Administradores'+d, title:' ',exportOptions: {columns: [0, 1, 2] },},
  ...
  <header class="ls-info-header">
    <!--icone no titulo -->
    <h2 class="ls-title-3 ls-ico-user">Administradores</h2>
    <a href="cadastro_adm.php" class="ls-btn ">Adicionar Administrador</a>
  </header>
...
<table  class="ls-table ls-table-striped display" id="tabela" style="width:100%">
  <thead>
    <tr>
      <th class="ls-data-descending">Empresa</th>
  ...
  <tbody>
    ...
    <tr>
     <?php
        ...
        do {
      ?>
      <td class="hidden-xs"><?=$linha['nomeEmpresa']?></td>
      ...
      <td>
        ...
        <form action="visualizar_adm.php" method="POST" enctype="multiparty/form-data">
        <input type="hidden" name="idAdministrador" id="idAdministrador" value=<?=$linha['idAdministrador']?>> 
        <button type="submit" class="ls-btn ls-ico-eye espacobtn" title="Ver"></button>
        </form>
        <form action="editar_adm.php" method="POST" enctype="multiparty/form-data">
        <input type="hidden" name="idAdministrador" id="idAdministrador" value=<?=$linha['idAdministrador']?>>
        <button  type="submit" class="ls-btn ls-ico-pencil espacobtn" title="Visualizar informações"></button>
        </form>
        <form action="include/Dadm.php" method="POST" enctype="multiparty/form-data">
        <input type="hidden"name="idAdministrador" id="idAdministrador" value=<?=$linha['idAdministrador']?>>
